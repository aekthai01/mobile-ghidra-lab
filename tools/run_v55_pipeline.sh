...
test -s "$OUT/v56_login_priority.json"
...
echo "[v5.6] selective Ghidra export + protected preservation + independent region C/P-code fallback"
"$GHIDRA_HOME/support/analyzeHeadless" "$PROJECT_DIR" "$PROJECT_NAME" \
  ...
  -postScript ExportV56RegionC.java "$OUT" "$OUT/v52_selected_functions.csv" 20 20000 \
  -deleteProject \
  > "$OUT/ghidra_selected.log" 2>&1
...
test -s "$OUT/v55_region_high_pcode_summary.csv"
test -s "$OUT/v56_region_c_index.csv"

echo "[v5.6] control-flow, C merge, and exhaustive C coverage validation"
...
test -s "$OUT/v56_c_coverage_acceptance.md"
...
test -s "$OUT/v54_protected_manifest.json"
test -s "$OUT/v55_region_high_pcode_acceptance.md"
test -s "$OUT/v55_region_high_pcode_merge.json"
test -s "$OUT/v55_region_map.csv"
test -s "$OUT/v55_reconstruction_summary.csv"
test -s "$OUT/v55_cfg_report.md"
test -s "$OUT/v55_native_data_report.md"
test -s "$OUT/v55_complete_views_report.md"
test -s "$OUT/v55_complete_views_stats.json"
test -s "$OUT/v55_semantic_c_report.md"
test -s "$OUT/v55_semantic_c_stats.json"
test -s "$OUT/v55_semantic_string_recovery.csv"
test -s "$OUT/v55_semantic_quality_v2.json"
test -s "$OUT/v55_semantic_quality_v2.md"
test -s "$OUT/v55_semantic_acceptance.md"
test -s "$OUT/v55_acceptance.md"
test -s "$OUT/v56_c_coverage_acceptance.json"
test -s "$OUT/v56_c_coverage_acceptance.md"
test -s "$OUT/human/START_HERE.html"
test -s "$OUT/human/V55_RECONSTRUCTION.html"
test -s "$OUT/human/V55_SEMANTIC_C.html"
test -s "$OUT/human/SEMANTIC_C_INDEX.csv"
test -s "$OUT/human/V55_CFG.html"
test -s "$OUT/human/V55_NATIVE_DATA.html"
test -s "$OUT/human/OFFSET_LOOKUP.html"
test -s "$OUT/human/OFFSET_MAP.txt"
test -s "$OUT/human/FUNCTIONS_C.html"
test -s "$OUT/human/FUNCTION_C_INDEX.csv"
test -s "$OUT/human/ida/import_mobile_ghidra.py"
test -s "$OUT/human/reconstructed_c_v56/V56_C_RECONSTRUCTION.md"
