#!/usr/bin/env python3
"""Fatal V5.5 AI-native semantic-C quality gate V3."""
...
prov=sum('Provenance/confidence:' in t for t in semtxt)
conf=sum(all(x in t for x in ('EXACT','STRONG','HEURISTIC')) for t in semtxt)
...
# Index address-bearing semantic lines for direct ARM64 preservation checks.
addr_lines={}
for t in comptxt:
 for line in t.splitlines():
  m=re.search(r'IDA/RVA 0x([0-9A-Fa-f]+)',line)
  if m:addr_lines.setdefault(int(m.group(1),16),[]).append(line)
BASE=0x100000; branch_total=branch_ok=call_total=call_ok=0; mod_total=mod_ok=0
asm_re=re.compile(r'^\.text:[0-9A-Fa-f]{16}\s+(?:(?:[0-9A-Fa-f]{2}\s+){4}\s*)?([A-Za-z0-9.]+)\s*(.*?)\s*(?:;.*IDA_RVA=0x([0-9A-Fa-f]+))?$',re.I)
for p in asm:
 for line in p.read_text(encoding='utf-8',errors='replace').splitlines():
  m=asm_re.match(line)
  if not m:continue
  mn=m.group(1).upper();op=m.group(2); am=re.search(r'IDA_RVA=0x([0-9A-Fa-f]+)',line,re.I)
  if not am:continue
  ...
  if mn in ('MOVI','MVNI') and re.search(r'\bLSL\s*#?(?:8|16|24)',op,re.I):
   mod_total+=1
   if any('modified-imm raw_bits=0x' in q and 'confidence=EXACT' in q for q in ls):mod_ok+=1
...
# Any denormal float literal in semantic output is suspicious after V3.
denorm=[]
...
summary_count=len(re.findall(r'/\* semantic:',joined,re.I));fallbacks=len(re.findall(r'\bARM64_[A-Z0-9_.]+\s*\(',joined))
report={'selected':len(selected),'semantic_c':len(idx),'cfg_blocks':len(rows('v55_cfg_blocks.csv')),'provenance':f'{prov}/{len(semtxt)}','confidence_policy':f'{conf}/{len(semtxt)}','arm64_ground_truth':f'{len(asm)}/{len(selected)}','pcode_noise':noise,'semantic_summaries':summary_count,'unsupported_explicit_arm64_fallbacks':fallbacks,'direct_branch_targets':f'{branch_ok}/{branch_total}','direct_branch_target_pct':round(branch_pct,4),'direct_calls':f'{call_ok}/{call_total}','direct_call_pct':round(call_pct,4),'modified_f32_immediates':f'{mod_ok}/{mod_total}','suspicious_denormal_splats':len(denorm),'stress_0029C094':stress_checks,'four_layers':four,'protected_manifest':f'{verified}/{len(manifest)}','errors':errors,'status':'pass' if not errors else 'fail'}
