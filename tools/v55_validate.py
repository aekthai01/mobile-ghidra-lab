...
complete=load_json('v55_complete_views_stats.json')
if not complete:errors.append('complete human views stats missing/invalid')
if complete:
 n=len(selected)
...
semantic=load_json('v55_semantic_c_stats.json');semacc=load_json('v55_semantic_acceptance.json')
if not semantic:errors.append('ARM64-grounded semantic C stats missing/invalid')
else:
 n=len(selected)
 if int(semantic.get('selected_functions') or -1)!=n:errors.append('semantic C selected-function count mismatch')
 if int(semantic.get('semantic_c_files') or -1)!=n:errors.append('semantic C coverage is not 100% of selected functions')
 if semantic.get('missing_functions'):errors.append('semantic C reports missing selected functions')
 if int(semantic.get('cfg_blocks_total') or 0)<=0:errors.append('semantic C CFG coverage is empty')
if not semacc:errors.append('semantic C acceptance missing/invalid')
elif semacc.get('status')!='pass':errors.append('semantic C acceptance did not pass')
report={'high_protected_functions':len(high),'protected_evidence_ok':sum(1 for e in high if e in v54 and (v54[e].get('status') or '')=='ok'),'high_pcode_rows':len(highp),'cfg_functions':len(cfg),'reconstructed_functions':len(recon),'native_sections':len(native),'selected_functions':len(selected),'complete_c_views':complete.get('c_views',0),'complete_asm_views':complete.get('selected_asm_views',0),'dual_address_offset_rows':complete.get('offset_index_rows',0),'semantic_c_views':semantic.get('semantic_c_files',0),'semantic_cfg_blocks':semantic.get('cfg_blocks_total',0),'semantic_idiom_folds':semantic.get('idiom_folds',0),'semantic_recovered_strings':semantic.get('recovered_string_refs',0),'errors':errors,'status':'pass' if not errors else 'fail'}
