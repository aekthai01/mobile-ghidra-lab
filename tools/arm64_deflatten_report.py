...
priority = {(r.get('entry') or '').upper(): r for r in priority_rows}
suspicious = {(r.get('entry') or '').upper(): r for r in suspicious_rows}
...
def parse_function(path):
    ...
    for line in path.read_text(encoding='utf-8', errors='replace').splitlines():
        ...
        m = INS_RE.match(line)
        if m:
            ins.append({'addr': int(m.group(1),16), 'bytes': m.group(2), 'mnem': m.group(3).upper(), 'ops': m.group(4).strip(), 'line': line})
    if not ins:
        return None
    if start is None:
        start = ins[0]['addr']
...
def build_cfg(fn):
    ...
    edges = []
    for block in blocks:
        s = block[0]['addr']; last = block[-1]; m = last['mnem']; target = direct_target(last['ops'])
        if is_cond(m):
            if target in block_by: edges.append((s,target,'branch'))
            ft = next_block[s]
            if ft is not None: edges.append((s,ft,'fallthrough'))
        elif m == 'B':
            if target in block_by: edges.append((s,target,'branch'))
    ...
    for a,b,k in edges:
        succ[a].append((b,k)); pred[b].append((a,k))
    return blocks, block_by, edges, succ, pred


def tarjan(nodes, succ):
    index = 0; stack = []; onstack = set(); idx = {}; low = {}; comps = []
...
def block_features(block, pred, succ):
    ...
    text = ' '.join(i['ops'] for i in block)
    regs_cmp = []
    compare_count = 0
    const_writes = Counter()
    for i in block:
        regs = REG_RE.findall(i['ops'])
        if i['mnem'] in CMPISH or i['mnem'] in {'CBZ','CBNZ','TBZ','TBNZ'}:
            compare_count += 1
            if regs: regs_cmp.append(regs[0].upper().replace('X','W'))
        if i['mnem'] in CONSTISH and regs:
            const_writes[regs[0].upper().replace('X','W')] += 1
    ...
    indeg = len(pred.get(start, [])); outdeg = len(succ.get(start, []))
    ...
    semantic = calls > 0 or stores > 0 or len(block) >= 8 or compare_count >= 2
    return {
        'start': start, 'instructions': len(block), 'indegree': indeg, 'outdegree': outdeg,
        'back_predecessors': backpred, 'compare_count': compare_count, 'compare_regs': regs_cmp,
        'constant_writes': const_writes, 'calls': calls, 'stores': stores, 'loads': loads,
        'indirect_jumps': indirect, 'trampoline': trampoline, 'semantic': semantic, 'text': text,
    }
...
def infer_indirect_context(block, idx):
    ...
    for ins in window:
        for m in TARGET_RE.finditer(ins['ops']):
            labels.append('0x' + m.group(1).upper())
...
report_dir.mkdir(exist_ok=True); dot_dir.mkdir(exist_ok=True)

# Candidate functions: suspicion first, then app-priority. Keep third-party only if it is very suspicious
# or close to an app seed.
all_paths = []
for p in asm_dir.glob('*.asm'):
    m = re.match(r'([0-9A-Fa-f]+)_', p.name)
    entry = m.group(1).upper() if m else ''
    pr = priority.get(entry, {})
    sus = suspicious.get(entry, {})
    try: sus_score = int(sus.get('suspicion_score') or 0)
    except ValueError: sus_score = 0
    try: pr_score = int(pr.get('analysis_priority') or 0)
    except ValueError: pr_score = 0
    third = (pr.get('is_third_party') or '').lower() == 'true'
    try: dist = int(pr.get('app_seed_distance')) if pr.get('app_seed_distance') not in (None,'') else 99
    except ValueError: dist = 99
    include = sus_score >= 20 or pr_score >= 1200 or dist <= 2
    if third and dist > 2 and sus_score < 60:
        include = False
    if include:
        all_paths.append((-(sus_score*100 + max(0,pr_score)), p))
all_paths.sort(key=lambda x: (x[0], x[1].name))
all_paths = [p for _,p in all_paths[:140]]
...
for path in all_paths:
    ...
    scc_size = {}
    for comp in comps:
        for n in comp: scc_size[n] = len(comp)
    features = {b[0]['addr']: block_features(b,pred,succ) for b in blocks}

    state_regs = Counter()
    dispatcher_scored = []
    for start, feat in features.items():
        for r in feat['compare_regs']: state_regs[r] += 1
        score = feat['indegree']*3 + feat['back_predecessors']*5 + feat['compare_count']*4
        if scc_size.get(start,1) >= 6: score += min(30, scc_size[start]//2)
        if feat['indirect_jumps']: score += 18
        if feat['outdegree'] >= 2: score += 5
        if feat['trampoline']: score -= 8
    ...
    # Build a collapsed direct CFG that bypasses trivial branch trampolines.
        ...
        for dst,kind in succ.get(src,[]):
            ...
            if final != src:
                ...
                clean_edge_rows.append({
                    'function_entry': f'{fn["start"]:08X}', 'function_name': fn['name'],
                    'from_block': f'{src:08X}', 'to_block': f'{final:08X}', 'kind': kind,
                    'collapsed_trampolines': str(final != dst).lower(),
                })

    # Collect indirect branches with a local data-flow context.
    function_indirect = []
    for block in blocks:
        for idx,item in enumerate(block):
            if item['mnem'] in INDIRECT_JUMPS:
                ...
                row = {
                    'function_entry': f'{fn["start"]:08X}', 'function_name': fn['name'],
                    'branch_address': f'{item["addr"]:08X}', 'mnemonic': item['mnem'],
                    'target_register': reg, 'nearby_symbolic_targets': ';'.join(labels),
                    'nearby_immediates': ';'.join(constants), 'context': context.replace('\n','\\n'),
                }
                indirect_rows.append(row); function_indirect.append(row)

    max_scc = max((len(c) for c in comps), default=0)
    try: sus_score = int(suspicious.get(f'{fn["start"]:08X}',{}).get('suspicion_score') or 0)
    except ValueError: sus_score = 0
    pr = priority.get(f'{fn["start"]:08X}', {})
    candidate_rows.append({
        'entry': f'{fn["start"]:08X}', 'name': fn['name'], 'size_bytes': fn['size'],
        'basic_blocks': len(blocks), 'direct_edges': len(edges), 'largest_scc': max_scc,
        ...
        'indirect_branches': len(function_indirect), 'suspicion_score': sus_score,
        'analysis_priority': pr.get('analysis_priority',''),
        'third_party_family': pr.get('third_party_family',''),
    })

    out = report_dir / f'{fn["start"]:08X}_{sanitize(fn["name"])}.md'
    with out.open('w', encoding='utf-8') as w:
        ...
        w.write('\nThese are reconstruction hints, not a claim that control-flow flattening is definitely present.\n\n')
        w.write('## Dispatcher candidates\n\n| Score | Block | In | Out | Back preds | Compares | SCC |\n|---:|---|---:|---:|---:|---:|---:|\n')
        for score,start in dispatchers:
            f = features[start]
            w.write(f'| {score} | `loc_{start:X}` | {f["indegree"]} | {f["outdegree"]} | {f["back_predecessors"]} | {f["compare_count"]} | {scc_size.get(start,1)} |\n')
        w.write('\n## State-register candidates\n\n')
        if top_state:
            for reg,count in top_state: w.write(f'- `{reg}` compared/tested **{count}** times\n')
        else:
            w.write('- No dominant compared register found.\n')
        w.write('\n## Likely semantic blocks\n\n')
        sem = sorted((features[n] for n in semantic_nodes), key=lambda x: (-(x['calls']*8+x['stores']*3+x['compare_count']*2+x['instructions']//8), x['start']))[:80]
        w.write('| Block | Insns | Calls | Stores | Compares | In | Out |\n|---|---:|---:|---:|---:|---:|---:|\n')
        for f in sem:
            w.write(f'| `loc_{f["start"]:X}` | {f["instructions"]} | {f["calls"]} | {f["stores"]} | {f["compare_count"]} | {f["indegree"]} | {f["outdegree"]} |\n')
        if function_indirect:
            ...
            for row in function_indirect[:20]:
                ...
                if row['nearby_symbolic_targets']: w.write(f'- Nearby symbolic targets: `{row["nearby_symbolic_targets"]}`\n')
                w.write('\n```asm\n' + row['context'].replace('\\n','\n') + '\n```\n\n')

    # DOT uses the collapsed graph, which is far more readable than raw flattened spaghetti.
    dot = dot_dir / f'{fn["start"]:08X}_{sanitize(fn["name"])}.dot'
    with dot.open('w', encoding='utf-8') as w:
        ...
        for n in semantic_nodes:
            f = features[n]
            attrs = []
            if n in dispatcher_nodes: attrs.append('penwidth=3')
            label = f'loc_{n:X}\\nins={f["instructions"]} calls={f["calls"]} stores={f["stores"]}'
...
if candidate_rows:
    with (root / 'arm64_deflatten_candidates.csv').open('w', encoding='utf-8', newline='') as f:
        w = csv.DictWriter(f, fieldnames=list(candidate_rows[0].keys())); w.writeheader(); w.writerows(candidate_rows)
if indirect_rows:
    with (root / 'arm64_indirect_branches.csv').open('w', encoding='utf-8', newline='') as f:
        w = csv.DictWriter(f, fieldnames=list(indirect_rows[0].keys())); w.writeheader(); w.writerows(indirect_rows)
if clean_edge_rows:
    with (root / 'arm64_clean_cfg_edges.csv').open('w', encoding='utf-8', newline='') as f:
        w = csv.DictWriter(f, fieldnames=list(clean_edge_rows[0].keys())); w.writeheader(); w.writerows(clean_edge_rows)
...
with (root / 'arm64_deflatten_report.md').open('w', encoding='utf-8') as w:
    ...
    w.write('\nOpen `arm64_deflatten/` for per-function reconstruction notes and `arm64_deflatten_svg/` for rendered simplified graphs when Graphviz is available.\n')
