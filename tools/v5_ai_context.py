...
selected = rows('v5_selected_functions.csv') or rows('v4_selected_functions.csv')
...
pcode_slices = rows('v5_pcode_indirect_slices.csv')
...
val_by = {(canon(r.get('function_entry')), canon(r.get('branch_address'))): r for r in validation}
pc_by = {(canon(r.get('function_entry')), canon(r.get('branch_address'))): r for r in pcode_slices}

callers = defaultdict(list)
callees = defaultdict(list)
for r in call:
    a = canon(r.get('caller_entry')); b = canon(r.get('callee_entry'))
    if a and b:
...
for rank, r in enumerate(selected[:80], 1):
    ...
    with card.open('w', encoding='utf-8') as w:
        ...
        if inds:
            ...
            for x in inds[:20]:
                key = (e, canon(x.get('branch_address')))
                vr = val_by.get(key, {})
                pr = pc_by.get(key, {})
                w.write(f'- `0x{x.get("branch_address")}` pattern=`{x.get("pattern")}` table=`{x.get("table_address")}` index=`{x.get("index_register")}` static-validation=`{vr.get("classification", "not-validated")}`\n')
                if pr.get('target_expression'):
                    w.write(f'  - P-code target expression: `{pr.get("target_expression")[:500]}`\n')
                    w.write(f'  - Slice: `{pr.get("slice_path", "")}`\n')
        if sts:
        ...
        if str_by.get(e):
            w.write('\n## Referenced strings\n\n')
            for s in str_by[e][:25]: w.write(f'- `{s[:240]}`\n')

        w.write('\n## Suggested next evidence\n\n')
        if r.get('recommended_mode') == 'region':
            w.write(f'- Open `v5_slices/{e}/`, then use `v5_pcode_slices/`, `v5_jump_table_validation.csv`, and `v5_clean_edges.csv`. Treat table-shaped ARM64 with invalid on-disk targets as a runtime/decryption lead, not a recovered jump table.\n')
        else:
            w.write('- Open the matching `v4_selected_ida/` and `v4_decompiled_selected/` files, then correlate callers/callees and P-code evidence.\n')
...
with (root / 'ai_context' / 'overview.md').open('w', encoding='utf-8') as w:
    ...
    w.write('Read this file first. The cards contain bounded evidence so a phone AI client does not need to ingest the entire artifact. Static jump-table claims are validated against real disassembled instruction addresses; invalid on-disk tables remain marked as runtime/transformed candidates.\n\n')
    w.write('| # | Score | Entry | Mode | Function | Card |\n|---:|---:|---|---|---|---|\n')
    for r in summary[:50]:
        w.write(f"| {r['rank']} | {r['score']} | `0x{r['entry']}` | {r['mode']} | `{r['name']}` | `{r['card']}` |\n")
    w.write('\nFor giant functions, prioritize semantic slices, P-code backward slices, and validated control-flow evidence over whole-function pseudocode. Ghidra-generated `FUN_...` names are identifiers, not recovered original names.\n')
